...
pub fn load_config() -> AppConfig {
  ...
  {
    dotenvy::from_path_override(config_path).ok();
    ...
    config.voice_id = std::env::var("VOICE_ID")
...
pub fn save_config(
  ...
  // Preserve voice fields — use existing values if the caller left them blank
  ...
  let voice_id = if config.voice_id.is_empty() {
    if existing_config.voice_id.is_empty() {
  ...
  let playback_speed = if config.playback_speed == 0.0 {
    if existing_config.playback_speed == 0.0 {
      1.0_f32
    } else {
