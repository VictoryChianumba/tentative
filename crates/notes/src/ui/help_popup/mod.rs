...
pub use global_bindings::GlobalBindings as HelpGlobalBindings;
pub use keybindings_table::CommandRow;
pub use multi_select_bindings::MultiSelectBindings as HelpMultiSelectBindings;
...
impl HelpPopup {
  pub fn new(selected_tab: KeybindingsTabs) -> Self {
    Self {
      selected_tab,
      global_bindings: GlobalBindings::new(),
      multi_select_bindings: MultiSelectBindings::new(),
    }
  }

  pub fn global_bindings_mut(&mut self) -> &mut GlobalBindings {
    &mut self.global_bindings
  }

  pub fn multi_select_bindings_mut(&mut self) -> &mut MultiSelectBindings {
    &mut self.multi_select_bindings
  }
  ...
  pub fn handle_input(&mut self, key: KeyEvent) -> HelpInputReturn {
    let has_control = key.modifiers.contains(KeyModifiers::CONTROL);
    match key.code {
      KeyCode::Esc | KeyCode::Char('q') => HelpInputReturn::Close,
      KeyCode::Char('c') if has_control => HelpInputReturn::Close,
      KeyCode::Char('g') => {
      ...
      KeyCode::Down | KeyCode::Char('j') => {
        match self.selected_tab {
          ...
          KeybindingsTabs::MultiSelect => {
            self.multi_select_bindings.select_next()
          }
        }
        HelpInputReturn::Keep
      }
      KeyCode::Up | KeyCode::Char('k') => {
        match self.selected_tab {
          ...
          KeybindingsTabs::Editor => {}
          KeybindingsTabs::MultiSelect => {
            self.multi_select_bindings.select_previous()
          }
        }
        HelpInputReturn::Keep
      }
      _ => HelpInputReturn::Keep,
    }
  }
}

fn render_keybindings<T: KeybindingsTable>(
  frame: &mut Frame,
  area: Rect,
  table: &mut T,
) {
  let header_cells =
    ["Key", "Command", "Description"].into_iter().map(|header| {
      Cell::from(header).style(Style::default().fg(Color::LightBlue))
    });
  ...
  let rows: Vec<Row> = table
    ...
    .map(|row| {
      ...
      let keys_text = textwrap::fill(row.key.as_str(), keys_width as usize);
      let name = textwrap::fill(row.name.as_str(), name_width as usize);
      let description =
        textwrap::fill(row.description.as_str(), description_width as usize);
